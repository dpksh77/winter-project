...
while(i<2):
	...
	if int(ch)==2:
		...
		print "\n  first we will transfer file to this ip "+ip
		os.system("scp -r "+output+"/configure/jobtrackerautomate  "+ip+":/root/Desktop")

		os.system("tput setaf 1")		
		print "\n now we are doing ssh means remote server process"
		
		print "\n after ssh you first type the command ]#cd /root/Desktop/jobtrackerautomate\n then type the command]#chmod +x hadoop_name_node_java.py\n then enter the command\n]#./hadoop_name_node_java.py"
		os.system("tput setaf 0")	
		os.system("ssh -X root@"+ip)		
...
#                                      data node coding
		...
		print "\n  first we will transfer file to this ip "+ip
		os.system("scp -r "+output+"/configure/tasktrackerautomate  "+ip+":/root/Desktop")

		os.system("tput setaf 1")		
		print "\n now we are doing ssh means remote server process"
		
		print "\n after ssh you first type the command ]#cd /root/Desktop/tasktrackerautomate\n then type the command]#chmod +x hadoop_data_node_java.py\n then enter the command\n]#./hadoop_data_node_java.py"
		os.system("tput setaf 0")	
		os.system("ssh -X root@"+ip)		
